...
export class CustomerService extends BaseService<Customer> {
  ...
  async createCustomer(createCustomerDto: CreateCustomerDto): Promise<CustomerResponseDto> {
    ...
    if (createCustomerDto.dynamicFields && createCustomerDto.dynamicFields.length > 0) {
      const dynamicFieldValues: CreateCustomerDynamicFieldValueDto[] = createCustomerDto.dynamicFields.map(field => ({
  ...
  async updateCustomer(id: number, updateCustomerDto: UpdateCustomerDto): Promise<CustomerResponseDto> {
    ...
    const customer = await this.update(updateCustomerDto, id, CustomerResponseDto);
    
    // Check for status change and fraud detection
    if (updateCustomerDto.status && oldStatus !== updateCustomerDto.status && updateCustomerDto.user) {
      // Log status change
      ...
      const uniqueCustomerChanges = await this.customerStatusChangeRepository.getUniqueCustomerChangesCount(
    ...
    if (updateCustomerDto.dynamicFields) {
      ...
      if (updateCustomerDto.dynamicFields.length > 0) {
        const dynamicFieldValues: CreateCustomerDynamicFieldValueDto[] = updateCustomerDto.dynamicFields.map(field => ({
          customer: id,
          customer_dynamic_field: field.customer_dynamic_field,
          file: field.file,
          name: field.name,
